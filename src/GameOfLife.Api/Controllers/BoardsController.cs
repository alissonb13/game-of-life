    ...
    public class BoardsController(
        ...
        IGetLastBoardState getLastBoardState,
        ILogger<BoardsController> logger) : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(GetLastBoardStateOutput), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> GetLatestBoardState([FromRoute] Guid boardId, [FromQuery] int generationMaxValue)
        {
            if (boardId != Guid.Empty)
                return Ok(await getLastBoardState.Execute(new GetLastBoardStateInput(boardId, generationMaxValue)));
